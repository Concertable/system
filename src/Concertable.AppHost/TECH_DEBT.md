...
**Progress:** the canonical pattern is established. **Done:** Search (`AddSearchWeb`/`AddSearchWorkers` in `Concertable.Search.AppHost.Extensions/SearchAppHostExtensions.cs`), Customer (`AddCustomerWeb` in `Concertable.Customer.AppHost.Extensions/CustomerAppHostExtensions.cs`), and Auth (`AddAuth` + `AddAuthTopology` in `Concertable.Auth.AppHost.Extensions/AuthAppHostExtensions.cs` + `AuthTopology.cs`) — every AppHost + each service's E2E helpers resolve them from there; the generic `WithOptionalEnvironment` and `AddSecrets` helpers are `public` in `AppHost.Shared` for per-service libs to compose. **Remaining (every per-service wrapper still in `AppHost.Shared`, one service per follow-up PR):** B2B — `AddApi`, `AddWorkers`, `AddB2BSeedingSimulator`, `AddMobileB2B`; Payment — `AddPaymentWeb`, `AddPaymentWorkers`, `AddStripeCli`; SPAs — `AddCustomerSpa`, `AddVenueSpa`, `AddArtistSpa`, `AddBusinessSpa`; mobile — `AddMobile`, `AddMobileCustomer`. The generic helpers `AddSqlServerContainer`, `AddServiceBus`, `AddAzureStorage`, `Topology`, `WithOptionalEnvironment`, and `AddSecrets` are cross-service and stay.
...
**Progress:** **Search**, **Customer**, and **Auth** are done — Search's constants live in `Concertable.Search.AppHost.Extensions/SearchConstants.cs` (`Database`, `WebResource`, `WorkersResource`, `ServiceName`), Customer's in `Concertable.Customer.AppHost.Extensions/CustomerConstants.cs` (`Database`, `WebResource`, `ServiceName`), Auth's in `Concertable.Auth.AppHost.Extensions/AuthConstants.cs` (`Database`, `Resource`, `ServiceName`); the dead `Ports` class (0 references) was deleted outright. **Remaining (all entries still in `AppHostConstants`, split with their service's wrappers above):** `Databases.{B2B, Payment}`; `ResourceNames.{B2BWeb, Workers, B2BSeedingSimulator, PaymentWeb, PaymentWorkers, StripeCli}`; `ServiceNames.{B2B, Payment}`.
