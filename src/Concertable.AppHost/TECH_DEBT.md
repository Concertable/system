...
**Progress:** the canonical hosting pattern is established by Payment: `AddPaymentWeb`/`AddPaymentWorkers`/`AddStripeCli` live in `Concertable.Payment.Hosting/AppHostExtensions.cs`, and every AppHost plus the E2E helpers consumes that library. **Functionally extracted but still using the superseded project/type naming (migrate one service per follow-up PR):** Search (`Concertable.Search.AppHost.Extensions/SearchAppHostExtensions.cs`), Customer (`Concertable.Customer.AppHost.Extensions/CustomerAppHostExtensions.cs`), and Auth (`Concertable.Auth.AppHost.Extensions/AuthAppHostExtensions.cs`); each becomes `Concertable.X.Hosting/AppHostExtensions.cs`. **Still in `AppHost.Shared` (extract directly to the hosting pattern):** B2B — `AddApi`, `AddWorkers`, `AddB2BSeedingSimulator`, `AddMobileB2B`; SPAs — `AddCustomerSpa`, `AddVenueSpa`, `AddArtistSpa`, `AddBusinessSpa`; mobile — `AddMobile`, `AddMobileCustomer`. The generic `AddSqlServerContainer`, `AddServiceBus`, `AddAzureStorage`, `Topology`, `WithOptionalEnvironment`, and `AddSecrets` helpers are cross-service and stay.
...
**Progress:** **Search**, **Customer**, **Auth**, and **Payment** are out of the shared god-bucket — Search's constants live in `Concertable.Search.AppHost.Extensions/SearchConstants.cs` (`Database`, `WebResource`, `WorkersResource`, `ServiceName`), Customer's in `Concertable.Customer.AppHost.Extensions/CustomerConstants.cs` (`Database`, `WebResource`, `ServiceName`), Auth's in `Concertable.Auth.AppHost.Extensions/AuthConstants.cs` (`Database`, `Resource`, `ServiceName`), and Payment's in `Concertable.Payment.Hosting/PaymentConstants.cs` (`Database`, `WebResource`, `WorkersResource`, `StripeCliResource`, `ServiceName`). Search, Customer, and Auth move with their hosting-library renames tracked above; the dead `Ports` class (0 references) was deleted outright. **Remaining (all entries still in `AppHostConstants`, split with their service's wrappers above):** `Databases.B2B`; `ResourceNames.{B2BWeb, Workers, B2BSeedingSimulator}`; `ServiceNames.B2B`.
