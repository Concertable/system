...
**Progress:** the canonical hosting pattern is established by Payment, Search, and Customer: `AddPaymentWeb`/`AddPaymentWorkers`/`AddStripeCli` live in `Concertable.Payment.Hosting/AppHostExtensions.cs`, `AddSearchWeb`/`AddSearchWorkers` in `Concertable.Search.Hosting/AppHostExtensions.cs`, and `AddCustomerWeb` in `Concertable.Customer.Hosting/AppHostExtensions.cs`, each consumed by every AppHost plus the E2E helpers. **Functionally extracted but still using the superseded project/type naming (migrate one service per follow-up PR):** Auth (`Concertable.Auth.AppHost.Extensions/AuthAppHostExtensions.cs`) becomes `Concertable.Auth.Hosting/AppHostExtensions.cs`. **Still in `AppHost.Shared` (extract directly to the hosting pattern):** B2B — `AddApi`, `AddWorkers`, `AddB2BSeedingSimulator`, `AddMobileB2B`; SPAs — `AddCustomerSpa`, `AddVenueSpa`, `AddArtistSpa`, `AddBusinessSpa`; mobile — `AddMobile`, `AddMobileCustomer`. The generic `AddSqlServerContainer`, `AddServiceBus`, `AddAzureStorage`, `Topology`, `WithOptionalEnvironment`, and `AddSecrets` helpers are cross-service and stay.
...
**Progress:** **Search**, **Customer**, **Auth**, and **Payment** are out of the shared god-bucket — Search's constants live in `Concertable.Search.Hosting/SearchConstants.cs` (`Database`, `WebResource`, `WorkersResource`, `ServiceName`), Customer's in `Concertable.Customer.Hosting/CustomerConstants.cs` (`Database`, `WebResource`, `ServiceName`), Auth's in `Concertable.Auth.AppHost.Extensions/AuthConstants.cs` (`Database`, `Resource`, `ServiceName`), and Payment's in `Concertable.Payment.Hosting/PaymentConstants.cs` (`Database`, `WebResource`, `WorkersResource`, `StripeCliResource`, `ServiceName`). Auth moves with its hosting-library rename tracked above; the dead `Ports` class (0 references) was deleted outright. **Remaining (all entries still in `AppHostConstants`, split with their service's wrappers above):** `Databases.B2B`; `ResourceNames.{B2BWeb, Workers, B2BSeedingSimulator}`; `ServiceNames.B2B`.
